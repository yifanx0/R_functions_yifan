...
df_diff = function(df_1, df_2, i = 1, j = nrow(df_1)) {
  ...
  if (min(nrow(df_1), nrow(df_2)) < max(i, j)) {
    warning("At least one of the two data frames does not have enough rows", call. = FALSE)
  } else if (all.equal(df_1[i:j], df_2[i:j]) == TRUE) {
    # this is for if identical
    message("All good! The rows checked are all identical :)")
    ...
    for (k in i:j) {
      if (all.equal(df_1[k], df_2[k]) != TRUE) {
        ...
        cat(paste0("row ", k, " is different, with the problem:", "\n",
